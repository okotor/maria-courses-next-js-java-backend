...
export default function LoginForm() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post("http://marian-courses-backend-java-production.up.railway.app/login", {
        email,
        password,
      });
  ...
  const handleGoogleLoginSuccess = async (tokenResponse) => {
    ...
    try {
      const res = await axios.post("http://marian-courses-backend-java-production.up.railway.app/google-login", {
        token: credential,
      });
      const data = res.data;
